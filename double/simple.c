...
static int run(const ISPCRTDeviceType device_type, unsigned int N) {
    ...
    double *a = (double *)calloc(2 * N, sizeof(double)), *b = a + N;
    for (int i = 0; i < N; i++)
        a[i] = i, b[i] = N - i;
    ...
    ISPCRTMemoryView a_dev = ispcrtNewMemoryView(device, a, sizeof(double) * N, &flags);
    ISPCRTMemoryView b_dev = ispcrtNewMemoryView(device, b, sizeof(double) * N, &flags);
