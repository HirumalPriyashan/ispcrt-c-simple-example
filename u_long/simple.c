...
static int run(const ISPCRTDeviceType device_type, unsigned int N) {
    ...
    unsigned long *a = (unsigned long *)calloc(2 * N, sizeof(unsigned long)), *b = a + N;
    for (int i = 0; i < N; i++)
        a[i] = i, b[i] = N - i;
    ...
    ISPCRTMemoryView a_dev = ispcrtNewMemoryView(device, a, sizeof(unsigned long) * N, &flags);
    ISPCRTMemoryView b_dev = ispcrtNewMemoryView(device, b, sizeof(unsigned long) * N, &flags);
