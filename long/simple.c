...
static int run(const ISPCRTDeviceType device_type, unsigned int N) {
    ...
    long *a = (long *)calloc(2 * N, sizeof(long)), *b = a + N;
    for (int i = 0; i < N; i++)
        a[i] = i, b[i] = N - i;
    ...
    ISPCRTMemoryView a_dev = ispcrtNewMemoryView(device, a, sizeof(long) * N, &flags);
    ISPCRTMemoryView b_dev = ispcrtNewMemoryView(device, b, sizeof(long) * N, &flags);
